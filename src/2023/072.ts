...
registry.addElement(2023, 7, 2, (input: string) => new Solver(input).solution)
...
class Solver {
    ...
    private static readonly cards = ["J", "2", "3", "4", "5", "6", "7", "8", "9", "T", "Q", "K", "A"]
    ...
    private static readonly parseLine = (input: string): Hand => {
        ...
        const jokers: Counter = { card: "J", count: 0}
        ...
        if (jokers.count === 5) {
            counters.push(jokers)
        } else {
            const best = (counters.sort((a, b) => b.count - a.count))[0]
            best.count += jokers.count
        }
        let type = "High card"
