...
registry.addElement(2023, 17, 1, (input: string) => new Solver(input).solution)
...
class Solver {
    ...
    private static readonly getNewBest = (grid: GridElement[][], best: Best): Best[] => {
        ...
        if (best.direction === "left" || best.direction === "right") {
            {
                const checkBest: Best = { ...best, direction: "up", stepsRemaining: 3, lossSoFar: best.lossSoFar + gridElement.loss }
                newBest.push(checkBest)
            }
        }
        ...
        if (best.direction === "up" || best.direction === "down") {
            {
                const checkBest: Best = { ...best, direction: "left", stepsRemaining: 3, lossSoFar: best.lossSoFar + gridElement.loss }
                newBest.push(checkBest)
            }
        }
        ...
        if (best.direction === "left" || best.direction === "right") {
            {
                const checkBest: Best = { ...best, direction: "down", stepsRemaining: 3, lossSoFar: best.lossSoFar + gridElement.loss }
                newBest.push(checkBest)
            }
        }
        ...
        if (best.direction === "up" || best.direction === "down") {
            {
                const checkBest: Best = { ...best, direction: "right", stepsRemaining: 3, lossSoFar: best.lossSoFar + gridElement.loss }
                newBest.push(checkBest)
            }
        }
    ...
    constructor(input: string) {
        ...
        const start: Best = { direction: "right", stepsRemaining: 3, lossSoFar: -grid[0][0].loss, x: 0, y: 0 }
        let bestResult = Infinity
        let bestToCheck = Solver.getNewBest(grid, start)
