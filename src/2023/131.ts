...
registry.addElement(2023, 13, 1, (input: string) => new Solver(input).solution)
            ...
            positionLoop: for (let i = 1; i < rows[0].length; i++) {
                mirrorLoop: for (let j = i - 1; j >= 0; j--) {
                    ...
                    for (let l = 0; l < rows.length; l++) {
                        if (rows[l][j] !== rows[l][k]) continue positionLoop
                    }
                }
                positionFound = i
                break positionLoop
            ...
            positionLoop: for (let i = 1; i < rows.length; i++) {
                mirrorLoop: for (let j = i - 1; j >= 0; j--) {
                    ...
                    for (let l = 0; l < rows[0].length; l++) {
                        if (rows[j][l] !== rows[k][l]) continue positionLoop
                    }
                }
                positionFound = i
                break positionLoop
